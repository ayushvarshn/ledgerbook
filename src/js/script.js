...
class LendingLedger {
	...
	// Data Management (Backend REST)
	async initFromDB() {
		const all = await APIAdapter.loadAll();
		this.customers = all.customers;
		this.loans = all.loans;
		this.transactions = all.transactions;
		this.rates = all.rates;
		this.currentCustomerId = 1;
		this.currentLoanId = 1;
		this.currentTransactionId = 1;
	}

	async saveData(key, data) { /* no-op: backend persistence */ }
	...
	async saveCustomer() {
		...
		try {
			if (customerId) customer.id = parseInt(customerId);
			const saved = await APIAdapter.upsertCustomer(customer);
			const idx = this.customers.findIndex(c => c.id === saved.id);
			if (idx >= 0) this.customers[idx] = saved; else this.customers.push(saved);
			this.renderCustomers();
			this.updateDashboard();
			this.closeModal('customer-modal');
		} catch (e) {
			alert('Could not save customer. Please ensure the backend is running.\n\nDetails: ' + (e && e.message ? e.message : e));
		}
	}
	...
	async deleteCustomer(customerId) {
		if (confirm('Are you sure you want to delete this customer? This will also delete all associated loans and transactions.')) {
			await APIAdapter.deleteCustomer(customerId);
			// Remove from local state
			const remainingLoanIds = new Set(this.loans.filter(l => l.customerId !== customerId).map(l => l.id));
			this.customers = this.customers.filter(c => c.id !== customerId);
			this.loans = this.loans.filter(l => remainingLoanIds.has(l.id));
			this.transactions = this.transactions.filter(t => remainingLoanIds.has(t.loanId));
			this.renderCustomers();
			this.renderTransactions();
			this.updateDashboard();
		}
	...
	async deleteLoan(loanId) {
		...
		if (confirm('Are you sure you want to delete this loan? This will also delete all associated transactions.')) {
			await APIAdapter.deleteLoan(loanId);
	...
	async saveTransaction() {
		...
		if ((transaction.type || '').toLowerCase() === 'return_items') {
			...
			if (loan && Array.isArray(loan.collateralItems) && selectedLabels.length) {
				...
				APIAdapter.upsertLoan(loan).then(updated => {
	...
	async deleteTransaction(transactionId) {
		if (confirm('Are you sure you want to delete this transaction?')) {
			...
			await APIAdapter.deleteTransaction(transactionId);
	...
	async updateRate(type) {
		...
		this.rates = await APIAdapter.setRates(this.rates);
		this.updateRates();
